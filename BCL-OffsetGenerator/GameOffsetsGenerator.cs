...
public class GameOffsetsGenerator
{
    ...
    private readonly string[] _classfile;

    private readonly MannifestInfo _manifest;
    ...
    private void HandleOffset(string propertyname)
    {
        switch (propertyname)
        {
            case "meetingHudState":
                ...
                break;
            case "allPlayersPtr":
                _offsets.allPlayersPtr[_offsets.allPlayersPtr.Count - 1] = GetOffsetFromClass("GameData", "AllPlayers");
                break;
            ...
            case "door_isOpen":
                ...
                break;
            case "mushroomDoor_isOpen":
                _offsets.mushroomDoor_isOpen = GetOffsetFromClass("MushroomWallDoor", "open");
                break;
            ...
            case "palette_shadowColor":
                ...
                break;
            case "lightRadius":
                _offsets.lightRadius = new List<long> { GetOffsetFromClass("PlayerControl", "myLight", "lightSource"), GetOffsetFromClass("LightSource", "LightRadius", "viewDistance") };
                break;
            ...
            case "player.localX":
                ...
                _offsets.player.localX[1] = GetOffsetFromClass("CustomNetworkTransform", "prevPosSent", "lastPosSent");
                break;
            case "player.localY":
                _offsets.player.localY[0] = GetOffsetFromClass("PlayerControl", "NetTransform");
                _offsets.player.localY[1] = GetOffsetFromClass("CustomNetworkTransform", "prevPosSent", "lastPosSent") + 4;
                break;
            case "player.remoteX":
                _offsets.player.remoteX[0] = GetOffsetFromClass("PlayerControl", "NetTransform");
                _offsets.player.remoteX[1] = GetOffsetFromClass("CustomNetworkTransform", "targetSyncPosition", "lastPosition");
                break;
            case "player.remoteY":
                _offsets.player.remoteY[0] = GetOffsetFromClass("PlayerControl", "NetTransform");
                _offsets.player.remoteY[1] = GetOffsetFromClass("CustomNetworkTransform", "targetSyncPosition", "lastPosition") + 4;
                break;
            case "player.inVent":
            ...
            case "player.outfit.colorId":
                ...
                if (_offsets.player.outfit.colorId[0] == -1)
                {
                    _offsets.player.outfit.colorId[0] = GetOffsetFromClass("NetworkedPlayerInfo.PlayerOutfit", "ColorId");
                }
                break;
            case "player.outfit.hatId":
                _offsets.player.outfit.hatId[0] = GetOffsetFromClass("GameData.PlayerOutfit", "HatId");
                if (_offsets.player.outfit.hatId[0] == -1)
                {
                    _offsets.player.outfit.hatId[0] = GetOffsetFromClass("NetworkedPlayerInfo.PlayerOutfit", "HatId");
                }
                break;
            case "player.outfit.skinId":
                _offsets.player.outfit.skinId[0] = GetOffsetFromClass("GameData.PlayerOutfit", "SkinId");
                if (_offsets.player.outfit.skinId[0] == -1)
                {
                    _offsets.player.outfit.skinId[0] = GetOffsetFromClass("NetworkedPlayerInfo.PlayerOutfit", "SkinId");
                }
                break;
            case "player.outfit.visorId":
                _offsets.player.outfit.visorId[0] = GetOffsetFromClass("GameData.PlayerOutfit", "VisorId");
                if (_offsets.player.outfit.visorId[0] == -1)
                {
                    _offsets.player.outfit.visorId[0] = GetOffsetFromClass("NetworkedPlayerInfo.PlayerOutfit", "VisorId");
                }
                break;
            case "player.outfit.playerName":
                {
                    ...
                    {
                        _offsets.player.outfit.playerName[0] = GetOffsetFromClass("NetworkedPlayerInfo.PlayerOutfit", "PlayerName");
                    }
                    break;
                }
    ...
    private Struct[] GeneratePlayerStruct()
    {
        ...
        if (lookup.playerLevel == -1)
        {
            lookup = new
            {
                PlayerId = GetOffsetFromClass("NetworkedPlayerInfo", "PlayerId"),
                outfitsPtr = GetOffsetFromClass("NetworkedPlayerInfo", "Outfits"),
                playerLevel = GetOffsetFromClass("NetworkedPlayerInfo", "PlayerLevel"),
                disconnected = GetOffsetFromClass("NetworkedPlayerInfo", "Disconnected"),
                rolePtr = GetOffsetFromClass("NetworkedPlayerInfo", "Role"),
                taskPtr = GetOffsetFromClass("NetworkedPlayerInfo", "Tasks"),
                dead = GetOffsetFromClass("NetworkedPlayerInfo", "IsDead"),
                _objectPtr = GetOffsetFromClass("NetworkedPlayerInfo", "_object"),
            };
        }
        var curOffset = 0L;
